...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!taskIdPattern.test(id)) throw new RequestError(400, 'This research task ID is invalid.', 'INVALID_INPUT');
    const account = await requireAccount();
    const result = await callResearch(`/v1/deepresearch/tasks/${id}/status`, 'GET', account.accessToken);
    if (!result || typeof result !== 'object' || Array.isArray(result)) {
      throw new RequestError(502, 'Valyu returned an unexpected research response.', 'PROVIDER_ERROR');
    }
    return json({ investigation: investigationFromTask(result as Record<string, unknown>) });
  } catch (error) {
