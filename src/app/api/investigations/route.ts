...
import { buildResearchNotification } from '@/lib/server/research-notification';
...
export async function GET() {
  try {
    ...
    if (!Array.isArray(result))
      throw new RequestError(
        502,
        'Valyu returned an unexpected research history.',
        'PROVIDER_ERROR',
      );
    const investigations = result
      ...
      .filter(
        (task) =>
          ...
          task.query.startsWith('Research target:'),
      )
...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success)
      throw new RequestError(
        400,
        'Choose a valid location, category and research mode, with instructions under 2,000 characters.',
        'INVALID_INPUT',
      );
    ...
    if (typeof result.deepresearch_id !== 'string') {
      throw new RequestError(
        502,
        'Valyu did not return a valid research task.',
        'PROVIDER_ERROR',
      );
    }
    return json(
      {
        investigation: investigationFromTask(result, input),
        notified: Boolean(alertEmail),
      },
      201,
    );
  } catch (error) {
